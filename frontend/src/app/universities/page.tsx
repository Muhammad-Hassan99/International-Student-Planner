...
function UniversitiesContent() {
    ...
    useEffect(() => {
        ...
        fetch(
            `https://international-student-planner-production-c1eb.up.railway.app/universities/${encodeURIComponent(country)}`
        )
            .then(res => res.json())
            .then(data => {
    ...
    if (!country) {
        return (
            <div className="flex flex-col items-center justify-center py-20">
                ...
                <Link
                    href="/"
                    className="px-6 py-3 bg-primary text-white font-bold rounded-xl shadow-md"
                >
                    Go to Destinations
                </Link>
            </div>
        ...
        <div className="max-w-7xl mx-auto px-4 py-12">
            <div className="text-center mb-12">
                ...
                <p className="text-slate-500 max-w-2xl mx-auto mb-8">
                    Browse the top institutions in your selected destination,
                    or choose "Help me decide" to let our AI pick the best one
                    for your profile.
                </p>
            ...
            {loading ? (
                <div className="flex flex-col justify-center items-center py-20 gap-4">
                    ...
                    <p className="text-slate-500 font-medium animate-pulse">
                        Fetching prestigious universities in {country}...
                    </p>
                </div>
            ) : (
                <>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        <Link
                            href={`/plan?country=${encodeURIComponent(country)}&university=Any`}
                            className="p-6 bg-primary/5 rounded-2xl border-2 border-primary/20 shadow-sm hover:shadow-lg hover:border-primary transition-all flex items-start gap-4 cursor-pointer group text-left"
                        >
                            ...
                            <div>
                                ...
                                <p className="text-sm font-medium text-slate-600 dark:text-slate-400">
                                    I am not sure yet. Let the AI recommend the
                                    best university for my budget and degree!
                                </p>
                            </div>
                        </Link>
                        ...
                        {universities
                            ...
                            .map((uni) => (
                                <Link
                                    href={`/plan?country=${encodeURIComponent(country)}&university=${encodeURIComponent(uni)}`}
                                    key={uni}
                                    className="p-6 bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-lg transition-all flex items-start gap-4 cursor-pointer group"
                                >
                    ...
                    {!searchTerm && universities.length === 0 && (
                        <div className="text-center mt-12 p-8 bg-slate-50 dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800">
                            ...
                            <h3 className="font-bold text-lg mb-2">
                                No specific universities found.
                            </h3>

                            <p className="text-slate-500 mb-6">
                                Our AI can still build your roadmap with dynamic
                                recommendations!
                            </p>

                            <Link
                                href={`/plan?country=${encodeURIComponent(country)}&university=Any`}
                                className="px-6 py-3 bg-primary text-white font-bold rounded-xl shadow-md hover:bg-primary/90 transition-colors"
                            >
